...
public class DeleteTest {
	...
	private final String SRC = ExcelUtils.DATA_SRC + "DELETE_TEST.xlsx";
...
//		driver.manage().window().maximize();
	...
	public void multipleLogin(String username, String password,String fullname,String email, String expected) throws InterruptedException {
		...
		data.setEmail(email);
		data.setAction("Test delete function"); // thay đổi mô tả cho phù hợp với trường hợp test
		data.setLogTime(new Date()); 
		data.setExpected(expected); // expected nhập vào từ file data
		WebElement message = driver.findElement(By.id("message"));
		Thread.sleep(3000);
		
			data.setActual(!String.valueOf(message).equals("")? "Xóa Thành công" : "Xóa Thất bại");
			assertEquals(expected,!String.valueOf(message).equals("")? "Xóa Thành công" : "Xóa Thất bại");
